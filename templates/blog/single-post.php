...
while (have_posts()) : the_post();
    ...
    <article  <?php post_class($layout . ' ' . 'content' . ' ' . implode(' ', jwLayout::content_width())) ?> id="post-<?php the_ID(); ?>" <?php if ($review) { ?> itemscope itemtype="http://schema.org/Review" <?php } ?> >
        ...
        if (!isset($post_meta['_use_featured'][0])) {
            $featured_img = get_option('post_use_featured', '1');
        ...
        <div class="entry-content" >
            ...
            <?php
            if ((get_option('post_author', '1') == '1') ||
                    (get_option('post_date', '1') == '1') ||
                    (get_option('post_ratings', '1') == '1') ||
                    (get_option('post_type_icon', '1') == '1') ||
                    (get_option('post_comments_count', '1') == '1') ||
                    (get_option('post_category', '1') == '1')) {
                ?>
                <ul class="blog-meta-info-top">

                    <?php if (get_option('post_type_icon', '1') == '1') { ?>
                        <?php
                    ...
                    <?php if (get_option('post_category', '1') == '1') { ?>
                        <li class="post-meta-category">
                            <span><?php _e('Posted in: ', 'genesis-block-theme'); ?></span>
                            <?php echo jwRender::get_meta_category(); ?>
                        </li>
                    <?php } ?>

                    <?php if (get_option('post_date', '1') == '1' || (get_option('post_author', '1') == '1')) { ?>
                        <li class="post-meta-author-date">
                            <?php if (get_option('post_author', '1') == '1') { ?>
                                <?php echo _e('Posted by: ', 'genesis-block-theme'); ?>
                                <?php echo jwRender::get_meta_author(); ?>
                            <?php } ?>
                            <?php if (get_option('post_date', '1') == '1') { ?>
                                <span class="date">
                    ...
                    <?php if (get_option('post_ratings', '1') == '1') { ?>    
                    ...
                    <?php if (get_option('post_comments_count', '1') == '1') { ?>  
                        <li class="post-meta-comments">
                            <?php echo get_comment_meta(); ?>
                        </li>    
                    <?php } ?>
                    <?php edit_post_link(__('Edit', 'genesis-block-theme'), '<li class="post-meta-edit">', '</li>'); ?>
                </ul>
            <?php } ?>
        ...
        <?php if (get_option('post_share') == '1') { ?>
            ...
            <div class="share_post" role="main">
                ...
                $media = is_array($media) ? urlencode($media[0]) : null;
                $desc = urlencode($post->post_excerpt);
                ?>
                <ul class="socialshare-icon">
                    <?php if (get_option('post_share_fb') == '1') { ?>
                        <li>
                            <a class="link-facebook" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=<?php echo esc_url($link); ?>&t=<?php echo esc_attr($title); ?>">
                    ...
                    <?php if (get_option('post_share_tw') == '1') { ?>
                        <li>
                            <a class="link-twitter" target="_blank" href="https://twitter.com/intent/tweet?original_referer=<?php echo esc_url($link); ?>&text=<?php echo esc_attr($title); ?>&url=<?php echo esc_url($link); ?>">
                    ...
                    <?php if (get_option('post_share_pi') == '1') { ?>
                        <li>
                            <a class="link-pinterest" target="_blank" href="http://www.pinterest.com/pin/create/button/?url=<?php echo esc_url($link); ?>&media=<?php echo esc_attr($media) ?>&description=<?php echo esc_attr($desc); ?>">
                    ...
                    <?php if (get_option('post_share_mail') == '1') { ?>
                        <li>
                            <a class="link-email" target="_blank" href="mailto:<?php echo get_option('post_share_mail_content', 'youremail@addresshere.com'); ?>?subject=<?php echo esc_attr($title); ?>&body=<?php echo urldecode($desc) . ' ' . $link; ?>">
        ...
        <?php if (get_option('blog_author') == '1') { ?>
            <div id="admin_info" role="main">
                <div class="about_author" itemtype="http://schema.org/Person" itemscope itemprop="author">
                    ...
                    <div class="author_link"><a href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>"><?php _e('About the author', 'genesis-block-theme'); ?></a></div>                    
                </div>
                                ...
                                <ul class="socialshare-icon">
                                    <li>
                                        <span class="follow-me-title">
                                            <?php _e("Follow me on:", "genesis-block-theme"); ?>
                                        </span>
                                    </li>
        ...
        <?php
        if (get_option('show_comments', '1') == '1') {
            comments_template();
        }
        ?>
    </article>
