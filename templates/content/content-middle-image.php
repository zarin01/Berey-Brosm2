...
<article id="post-<?php the_ID(); ?>"  <?php post_class(array('element', $class, 'col-lg-4', 'content-middle', 'format-image')); ?>   
         ...
         sort_popular="<?php echo esc_attr($sort_num . get_comments_number());     //if(get_option('fbcomments_switch','0')=='0'){echo get_comments_number(); }else{echo jwFacebook::get_fb_comments_count(get_the_ID()); }                     ?>"
    ...
    <div class="box ">
        <div class="image <?php echo has_post_thumbnail() === false ? 'no-image-added' : 'image-added' ?>">
            <?php
            switch (get_option('std_post_image_clickable', '0')) {
            ...
            if (get_option('std_post_image_clickable', '0') != '0') {
                echo '</a>';
            }
            ?>
        </div>
                        ...
                        <?php } ?>
                            ...
                            <li class="post-meta-author-date">
                                <?php if (jaw_template_get_var('blog_meta_author', '1') == '1') { ?>                        
                                    <?php
                                    $author = _e('Posted by: ', 'genesis-block-theme') . ' ' . jwRender::get_meta_author();
                                    ?>
                                <?php } ?>
