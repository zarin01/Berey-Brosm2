...
$right_area = 0;
if (get_option('top_bar_login_button', '0') == '1') {
    $right_area = $right_area + 2;
}
if (get_option('top_bar_wishllist', '1') == '1') {
    $right_area = $right_area + 2;
}
if (get_option('top_bar_compare', '0') == '1') {
    $right_area = $right_area + 2;
}
if (get_option('top_bar_cart', 'off') == 'woo') {
    $right_area = $right_area + 3;
}
if (get_option('top_bar_search', '1') == '1') {
    $right_area = $right_area + 3;
}
if($right_area > 9){
    $right_area = 9;
...
if (get_option('top_bar_login_button', '0') == '0' && get_option('top_bar_wishllist', '1') == '0' && get_option('top_bar_cart', 'off') == 'off' && get_option('top_bar_search', '1') == '0') {
    $show_left_area = 'show-mobiles';
}
?>
<div class="col-lg-12  top-bar-1-left">
    <?php $icon = get_option('top_bar_span_icon', ''); ?>
    <?php if ($icon != '') { ?>
        <span class="top-bar-icon <?php echo $icon; ?>"></span>
    <?php } ?>
    <?php $text = get_option('top_bar_span_text', ''); ?>
    <?php if ($text != '') { ?>    
...
<div class="col-lg-<?php echo $right_area; ?> top-bar-1-right">
    <ul>
        <?php if (get_option('top_bar_login_button', '0') == '1') { ?>
            <li class="top-bar-login-content" aria-haspopup="true">
                ...
                if (is_user_logged_in()) {
                    $text = __('My account', 'genesis-block-theme');
                } else {
                    $text = __('Login', 'genesis-block-theme');
                }
                ?>
                ...
                ?>
                <div class="top-bar-login-form <?php echo esc_attr($class); ?>">
                    ...
                    <?php if (get_option('users_can_register') && !is_user_logged_in()) : ?>
                        <p class="regiter-button">
                            ...
                            ?>
                            <?php echo apply_filters('register', sprintf('<a class="btnregiter" href="%s">%s</a>', $register_link, __('Register', 'genesis-block-theme'))); ?>
                        </p>
                    <?php endif; ?>
                </div>                
            </li>
        ...
        <?php if (class_exists('YITH_Woocompare') && class_exists('WooCommerce')) { ?>
            <?php if (get_option('top_bar_compare', '0') == '1') { ?>
        ...
        <?php if (class_exists('YITH_WCWL') && class_exists('WooCommerce')) { ?>
            <?php if (get_option('top_bar_wishllist', '1') == '1') { ?>
        ...
        <?php if (class_exists('WooCommerce')) { ?>
            <?php if (get_option('top_bar_cart', 'off') == 'woo') { ?>
        ...
        <?php if (get_option('top_bar_search', '1') == '1') { ?>    
            <li>
                <?php
                if (get_option('top_bar_search_type', 'wordpress') == 'woo' && class_exists('WooCommerce')) {
