...
if (isset($options['breadcrumbs-enable']) && $options['breadcrumbs-enable'] === true) {
    ...
    echo '<span itemprop="title">' . __('Home', 'genesis-block-theme') . '</span>';
    echo "</a>";
    ...
    if (is_tag()) {
        echo '<a href="#">' . __('Tag:', 'genesis-block-theme') . ' ' . single_tag_title('', FALSE) . '</a>';
    } else if (is_author()) {
        $curauth = get_user_by('id', get_query_var('author'));
        echo '<a href="#">' . __('Author:', 'genesis-block-theme') . ' ' . $curauth->nickname . '</a>';
    } else if (is_year()) {
        ...
        } else if (function_exists('wc_get_page_id') && wc_get_page_id('shop') > 0 && is_shop()) {
            echo '<a href="' . get_the_permalink(wc_get_page_id('shop')) . '">' . __('Shop', 'genesis-block-theme') . '</a>';
        } else {
        ...
        if (function_exists('wc_get_page_id') && wc_get_page_id('shop') > 0 && !is_shop() && get_option('woo_breadcrumbs_show_shop', '0') == '1') {
            echo '<a href="' . get_the_permalink(wc_get_page_id('shop')) . '">' . __('Shop', 'genesis-block-theme') . '</a>';
        }
        ...
        if (!empty($terms) && isset($terms->term_id)) {
            ...
            $term_parent[] = $parent;
            if (isset($parent->parent) && is_array($parent->parent) && sizeof($parent->parent) > 0) {
        ...
        if (isset($terms->name)) {
            echo '<a href="#">' . __('Tag:', 'genesis-block-theme') . ' ' . $terms->name . '</a>';
        }
        ...
        echo '<a href="#">' . __('File not found', 'genesis-block-theme') . '</a>';
    } else if (is_search()) {
        echo '<a href="#">' . __('Search:', 'genesis-block-theme') . ' ' . get_search_query() . '</a>';
        ...
        if (isset($cats[0])) { // pokud neni v kategorii post neyobrazi se breadcrumb
            ...
            if (is_array($category) && sizeof($category) > 0 && $category != '') {
    ...
    if ((is_page() || is_single()) && !is_front_page()) {
        ...
        if ($switch) {
            echo '<a href="' . get_permalink() . '">' . jwUtils::crop_length(strip_tags(get_the_title()), get_option('blog_cut_breadcrumb', 50)) . '</a>';
        }
    }
}
