    ...
    <div class="col-lg-<?php echo jaw_template_get_var('box_size') . ' ' . $class; ?> builder-img">
        <?php if (jaw_template_get_var('link', '') != '' && jaw_template_get_var('lightbox', '0') == '0') { ?>
            ...
            <?php } else if (jaw_template_get_var('lightbox', '0') == '1') { ?>
                <a href="<?php echo $jaw_data["data"]["image"][0]->url; ?>" rel="prettyPhoto[<?php echo esc_attr($id); ?>]" title="<?php echo jaw_template_get_var('alt'); ?>">
                <?php } ?>
                <span id="image-<?php echo esc_attr($id); ?>"  >   
                    <?php $img_size = jaw_template_get_var('url-size', array('height' => '100%', 'width' => '100%')); ?>
                    <?php if ($jaw_data["data"]["image"][0]->url != '') { ?>
                        <img class="builder-image lazy" src="<?php echo $jaw_data["data"]["image"][0]->url; ?>"  alt="<?php echo jaw_template_get_var('alt'); ?>" title="<?php echo jaw_template_get_var('caption'); ?>" height="<?php echo $img_size['height']; ?>" width="<?php echo $img_size['width']; ?>"/>  
                    <?php } ?>
                </span>
            ...
            <?php if (jaw_template_get_var('hover_url', '') != '') { ?>
                <?php if (jaw_template_get_var('lightbox', '0') == '1') { ?>
                    <a href="<?php echo jaw_template_get_var('hover_url'); ?>" rel="prettyPhoto[<?php echo esc_attr($id); ?>]" title="<?php echo jaw_template_get_var('hover_caption'); ?>">
                    <?php } ?>
