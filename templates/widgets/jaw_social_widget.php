    ...
    foreach ($social_active as $service => $vars) {
        ...
        if ($vars["type"] == 'user') {
            if (!empty($vars["var"])) {
                ...
                $username_hash = Base62x::encode($vars["var"]);
                ...
                if (isset($service_vars)) {
                    ...
                    if (isset($service_vars->error) && $service_vars->error != '' && $show_errors === '1') {
                        ...
                        if (isset($instance['animated']) && $instance['animated'] == '1') {
                            ...
                            echo esc_attr($service_vars->followers);
                        }
                    }
                    echo "<span>";
                    switch ($service) {
                        case "google": _e("followers", "genesis-block-theme");
                            break;
                        case "twitter": _e("followers", "genesis-block-theme");
                            break;
                        case "facebook": _e("fans", "genesis-block-theme");
                            break;
                        case "instagram": _e("followers", "genesis-block-theme");
                            break;
                        case "flickr": _e("photos", "genesis-block-theme");
                            break;
                        case "youtube": _e("subscribers", "genesis-block-theme");
                            break;
                        case "vimeo": _e("followers", "genesis-block-theme");
                            break;
                        case "tumblr": _e("followers", "genesis-block-theme");
                            break;
                    }
                    echo "</span>";
                    echo "</a>";
