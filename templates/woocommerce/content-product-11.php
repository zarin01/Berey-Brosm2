...
if (class_exists('WooCommerce')) {
    ...
    $catalog_mode_class = '';
    if (get_option('woo_catalog') == '1') {
        $catalog_mode_class = 'catalog_mode_on';
    }
    $woo_signedin_price=true;
    if(get_option("woo_signedin_price",0) == 1 && !is_user_logged_in()) {
        $woo_signedin_price=false;
    }
    $thirspartyplugins = false;
    if (class_exists('gema75_woocommerce_badges')) {
        $thirspartyplugins = true;
    }
    ?> 

    <!-- woo fix 3.3.0 article class -->  
    <article id="product-<?php the_ID(); ?>"  <?php post_class(array('element', 'col-lg-2', 'product-style-11', 'product-animate', $catalog_mode_class)); ?>   
             ...
             sort_popular="<?php echo esc_attr(get_comments_number());     //if(get_option('fbcomments_switch','0')=='0'){echo get_comments_number(); }else{echo jwFacebook::get_fb_comments_count(get_the_ID()); }                ?>"
             >
        <div class="box ">
            <?php do_action('woocommerce_before_shop_loop_item'); ?>
            <?php
            if (get_option('woo_animation', 'simple') != 'off') {
                $gallery = $product->get_gallery_image_ids();
                if (isset($gallery[0])) {
                    $class = 'hower_image_' . get_option('woo_animation', 'simple');
                }
            }
            ?>
            ...
            <div class="product-box">
                <h2>
                    <a href="<?php echo esc_url($link); ?>" title="<?php echo esc_attr(get_the_title()); ?>" class="post_name"><?php echo jwUtils::crop_length(get_the_title(), get_option('letter_excerpt_title', -1)); ?></a>
                </h2>   <!-- Title -->
                <?php echo jwUtils::crop_length(jwRender::get_the_excerpt(), get_option('letter_excerpt', -1)); ?>
